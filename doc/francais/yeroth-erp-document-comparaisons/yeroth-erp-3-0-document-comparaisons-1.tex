...
\newcommand{\yerotherpblack}{\texttt{YEROTH--ERP--$3.0$}\xspace}
...
{\bf \Large \yerothrd} {| \sc \scriptsize Comparaisons \yerotherpblack, et autres solutions de gestion de stocks}			
\\ \line(1,0){540}
...
\begin{tabular}{ll}
\parbox{27em}{
\yerotherp est un \logicielerp (Progiciel de Gestion Intégré (\textbf{PGI}))
avantageux par rapport aux autres méthodes de 
gestion de stocks\`a cause des raisons suivantes:

\begin{enumerate}[1.]
	\itemsep -0.1em
	\item système-logiciel ERP, PGI, réseau compris
...
\caption*{La fiche des stocks (SOLUTION INFORMATISÉE 'YEROTH--ERP--3.0'}
\end{center}
}
\end{tabular}
\end{table}
...
\textbf{1. (nombres de succursales élevée)}		&
...
\textbf{1. (quantité totale d'articles à vendre, transférer)}&
		\yerothorange{très difficile}					&
		\yerothorange{très difficile}					&						
		\yerothvert{facile}						\\ \hline
				
\textbf{2. (travail simultanée et coordonées sur les données)}	&
		\yerothrouge{extrênement difficile}						&
		\yerothorange{très difficile}							&						
		\yerothvert{facile}				   					   \\
\end{tabular}}
\caption{Comparaison entre \yerotherp et les autres solutions de gestion de stocks\\}
...
USAGE D'1 PROGICIEL INTÉGRÉ DE GESTION POUR 1 RENTABILITÉ
EFFICACE !}
