...
\newcommand{\yerothpgitroiszero}{\textcolor{yerothColorBlue}{\sc YEROTH--PGI--$3.0$}\xspace}

\newcommand{\yerothpgiblack}{YEROTH--PGI--$3.0$\xspace}

\newcommand{\yerothpgi}{\textcolor{yerothColorBlue}{\sc YEROTH--PGI--$3.0$}\xspace}

\newcommand{\erp}{PROGICIEL DE GESTION INTÉGRÉ (PGI)\xspace}
...
\hypersetup{
    ...
    bookmarks,
    plainpages=false,
	pdftitle={Guide d'installation pour le \erp \yerothpgi (standalone) par: ''\myfullacademicname''},
...
\rhead{Guide d'installation pour le \erp \yerothpgiblack}
...
{\bf \Large \yerothrd} {| \sc \scriptsize guide d'installation de \yerothpgiblack}
\\ \line(1,0){540}

\vspace{2.0em}

\begin{center}
{\Large GUIDE D'INSTALLATION POUR LE PROGICIEL DE GESTION INTÉGRÉ (PGI) \yerothpgiblack}
\end{center}
...
$2018.20190227-2$	\\ 			
\end{tabular}
\caption{Logiciels requis pour l'installation de \yerothpgitroiszero.}
...
\begin{enumerate} [1)]
	...
	\item installer enfin \yerothpgitroiszero (\qt et \texlive sont install\'e automatiquement).
\end{enumerate}
...
\begin{enumerate}[1)]
	\item Editer le fichier (donn\'e avec \yerothpgitroiszero)
	...
	\item enfin ex\'ecuter le fichier \bash (donn\'e avec \yerothpgitroiszero)
		''yeroth-erp-3-0-configure-mysql-server.sh''.\\	
\end{enumerate}

\subsection{Installation de \qt, \texlive, et de \yerothpgitroiszero}
...
Apr\`es l'installation avec succ\`es de \yerothpgitroiszero,
