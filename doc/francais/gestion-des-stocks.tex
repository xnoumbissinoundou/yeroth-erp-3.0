...
\utilisateurs: \liencaissier, \lienmagasinier, \lienpatron.\\
...
\centering
\begin{tabular}{lcccc}
\textbf{T\^aches} 								& \patronb			& \gestionairedestocksb	& \magasinierb		& \caissierb 	 \\ \hline
...
\index{t\^aches de gestion des ventes}
\end{table}

Les fonctionalit\'es de gestion des stocks sont accessible
exclusivement aux \roles \magasinier, \caissier et \patron. Ces
...
\nxsection{Entrer un (nouveau) stock}
\index{entrer un (nouveau) stock}
\index{stock minimum}

\yeren entretient les donn\'ees suivantes pour chaque stock:
\begin{enumerate}[1)]
	\item la cat\'egorie
	\item la d\'esignation
	\item la date de p\'eremption du stock \optionel
	\item le fournisseur \optionel
	\item la localisation du stock
	\item le montant de la TVA d'un article \optionel
	\item le prix de vente d'un article
	\item la quantit\'e intiale (nombre de lots, et quantit\'e par lot)
	\item la \reference
...
Il existe deux m\'ethodes pour entrer un nouveau stock dans \yeren:
\begin{enumerate}[1)]
	...
	et la \yerenfield{\reference} d'un stock de m\^eme nature
	d\'ej\`a existant.\\
\end{enumerate}
...
\begin{itemize}[\mycheckmark{purplish}]
	\item \textcolor{purplish}{$\mathbf{1^{\text{\`ere}}}$ \textbf{m\'ethode}}\\
		   ...
		   sur le boutton \bouton{Entrer un nouveau stock} pour obtenir un
	...
	\item \textcolor{purplish}{$\mathbf{2^{\text{\`eme}}}$ \textbf{m\'ethode}}
		\begin{enumerate}[1)]
			...
			\item enfin, cliquez sur le le boutton \bouton{Entrer un nouveau stock}.\\
		\end{enumerate}				
...
\subsection{Lister les stocks avec le r\^ole \patron}
Pour le \role \patron, il existe deux m\'ethodes pour
lister les stocks dans \yeren \`a partir de la fen\^etre
